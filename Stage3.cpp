...
void CStage3::StartStage()
{
	...
	D_GAMEOBJMGR->CreateObj( EGAMEOBJ_CLEAR, L"3스테이지", L"3스테이지", D3DXVECTOR2( 358, 81 ), 0, false, ERENDER_LEFTUP, 1, 0, false );
	...
	D_PUZZLEMGR->CreatePuzzle( 8, 8, D3DXVECTOR2( 318, 617 ) );
}

void CStage3::OnFrameMove( float fElapsedTime )
{
	...
	if( D_GAMESTAGEMGR->GetGameTimmer() <= 0.0f )
	{
		...
		else
		{
			...
			else
			{
				...
				if( m_fEndTime >= 2.0f )
				{
					D_GAMESTAGEMGR->ChangeStage( ESTAGE_SCORE );
					return;
				}
			}
		}
