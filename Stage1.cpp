...
void CStage1::StartStage()
{
	...
	D_GAMEOBJMGR->CreateObj( EGAMEOBJ_CLEAR, L"1스테이지", L"1스테이지", D3DXVECTOR2( 430, 152 ), 0, false, ERENDER_LEFTUP, 1, 0, false );
	...
	D_PUZZLEMGR->CreatePuzzle( 6, 6, D3DXVECTOR2( 385, 555 ) );
}
	...
	if( D_GAMESTAGEMGR->GetGameTimmer() <= 0.0f )
	{
		...
		else
		{
			...
			else
			{
				...
				if( m_fEndTime >= 2.0f )
				{
					D_GAMESTAGEMGR->ChangeStage( ESTAGE_STAGE2 );
					return;
				}
			}
		}
